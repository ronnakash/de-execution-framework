...
class ContainerHarness:
    ...
    async def _setup(self) -> None:
        ...
        def _make_secrets(suffix: str) -> EnvSecrets:
            # Shared consumer group per module type: all parallel test
            # instances share the same group, so each Kafka message is
            # consumed exactly once. With messages keyed by tenant_id:symbol
            # and unique tenant_ids per test, results are isolated at the
            # data level.
        ...
        # Wait for Kafka consumers to get partition assignments.
        # With auto.offset.reset=latest, messages produced before assignment
        # are lost. We verify readiness by producing a sentinel message and
        # waiting for it to flow through the pipeline into ClickHouse.
    ...
    async def _wait_for_consumer_readiness(self) -> None:
        """Produce sentinel events until one appears in ClickHouse.

        Produces a fresh sentinel every ~3s with a unique ID. This handles the
        race where the consumer hasn't gotten its partition assignment yet when
        the first sentinel is produced (auto.offset.reset=latest would skip it).
        """
        ...
        while loop.time() < deadline:
            ...
            sentinel = {
                "id": sentinel_id,
                "tenant_id": f"{self.tenant_id}_SENTINEL",
                "status": "new",
            ...
            # Wait up to 3s for this sentinel to appear
            inner_deadline = loop.time() + 3.0
        ...
        raise TimeoutError(
            "Consumer readiness check failed: sentinel events did not appear "
            "in ClickHouse within 30s. Kafka consumers may not have partition "
            "assignments."
        )
...
class SubprocessHarness:
    ...
    async def _wait_for_consumer_readiness(self) -> None:
        ...
        Produces a fresh sentinel every ~3s with a unique ID. This handles the
        race where subprocess consumers haven't gotten partition assignments yet
        when the first sentinel is produced (auto.offset.reset=latest would skip it).
        """
        from de_platform.pipeline.topics import TRADE_NORMALIZATION
        ...
        while loop.time() < deadline:
            ...
            # Wait up to 3s for any sentinel to appear
            inner_deadline = loop.time() + 3.0
            while loop.time() < inner_deadline:
                rows = self._clickhouse_db.fetch_all("SELECT * FROM orders")
                if any(r.get("id") in seen_ids for r in rows):
