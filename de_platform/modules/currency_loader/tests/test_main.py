...
async def _run(rates_file: str, data: bytes) -> tuple[int, MemoryDatabase]:
...
async def test_currency_loader_inserts_rates():
    rates = [
        {"from_currency": "EUR", "to_currency": "USD", "rate": 1.08},
        {"from_currency": "GBP", "to_currency": "USD", "rate": 1.27},
    ]
    rc, db = await _run("rates.json", json.dumps(rates).encode())
    assert rc == 0
...
async def test_currency_loader_uppercases_currency_codes():
...
async def test_currency_loader_skips_incomplete_records():
    rates = [
        {"from_currency": "EUR", "to_currency": "USD", "rate": 1.08},
...
async def test_currency_loader_empty_file_returns_zero():
    rc, db = await _run("rates.json", json.dumps([]).encode())
    assert rc == 0
    assert db._tables.get("currency_rates", []) == []


async def test_currency_loader_missing_rates_file_raises():
    from de_platform.modules.currency_loader.main import CurrencyLoaderModule
